...
2.  Next, we also collect comprehensive number of GIS and other variables that relate to air pollutions, such as traffic density, road networks, and land uses.
...
epa_data <- read.csv("/data/ad_viz_plotval_data.csv")
...
pm_annual <- pm_daily %>%
  group_by(Site.ID) %>%
  summarise(Annual.Mean.PM25 = mean(Daily.Mean.PM2.5.Concentration, na.rm = TRUE)) %>%
  left_join(
    pm_daily %>% distinct(Site.ID, .keep_all = TRUE) %>% select(-c(Date, Daily.Mean.PM2.5.Concentration)), by = "Site.ID")
```

#### Mapping the annual data
```{r}
# Convert annual data into a spatial data
pm_annual.sf <- st_as_sf(pm_annual, coords = c("Site.Longitude","Site.Latitude"),  crs = 4326)

library(mapview)
mapview(pm_annual.sf, zcol = "Annual.Mean.PM25", legend = TRUE, layer.name = "Annual Mean PM2.5 (µg/m³)")
```

### Calculate distnance from surrounding environemnts 
